...
def apply_freeze_strategy(model: Autoencoder, strategy: str, verbose: bool = True):
    ...
    if strategy_normalized in ['full', 'none']:
        ...
        # 策略3：冻结底层
        # Select actual Linear modules instead of positional indices so that
        # activations or optional dropout layers cannot change the strategy.
        encoder_linear_layers = [
            layer for layer in model.encoder.children()
            if isinstance(layer, nn.Linear)
        ]
        for layer in encoder_linear_layers[:2]:
