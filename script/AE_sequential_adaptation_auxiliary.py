...
cumulative target states comprising the Local Structural Stiffening state, sensor
relocation, and the frozen span-20 temperature-conditioned nonlinear sensor
response.
...
VAL_SAMPLES_DR = 100     # filtered cumulative states: 394 train + 100 control
...
# Prescribed cumulative states: repair -> relocation -> temperature response
# Stage 1 (DR):       local stiffening only
# Stage 2 (DR+SO):    local stiffening + sensor relocation
# Stage 3 (DR+SO+TR): local stiffening + relocation + temperature response
DOMAINS = {
    "Source": {
    ...
    "DR": {
        "healthy": "damage_repaired_12_sequential_valid",
    ...
    "SO": {
        "healthy": "damage_repaired_12_offset_count_1_sequential_valid",
    ...
    "SD": {
        "healthy": "damage_repaired_12_offset_count_1_temperature_response_span20_sequential_valid",
        "damage": "second_damage_12_offset_count_1_temperature_response_span20_100",
        "val_samples": VAL_SAMPLES_DR,
        "label": "Repair + Relocation + Temperature Response",
        "short": "DR+SO+TR",
    },
}
...
def load_domain_data():
    ...
    if not os.path.isfile(DATA_AUDIT_PATH):
        raise FileNotFoundError(
            "Cumulative-data audit is missing; run "
            "AE_gen_sequential_cumulative_data.py before sequential training: "
            f"{DATA_AUDIT_PATH}"
        )
    with open(DATA_AUDIT_PATH, encoding="utf-8") as stream:
        data_audit = json.load(stream)
    if not data_audit.get("matched_control_damage_temperature_conditions", False):
    ...
    target_keys = ["DR", "SO", "SD"]
    target_train_shapes = {data[key]["train"].shape for key in target_keys}
    target_val_shapes = {data[key]["val"].shape for key in target_keys}
    target_damage_shapes = {data[key]["damage"].shape for key in target_keys}
    if len(target_train_shapes) != 1 or len(target_val_shapes) != 1 or len(target_damage_shapes) != 1:
        raise AssertionError(
            "CDR, CSO and CSD must use identical train/control/damage sample counts"
        )
    expected_train = int(data_audit["training_samples_per_cumulative_state"])
    expected_control = int(data_audit["final_healthy_control_samples_per_state"])
    expected_damage = int(data_audit["damage_samples_per_state"])
    if data["DR"]["train"].shape[0] != expected_train:
        raise AssertionError("Sequential training count does not match the data audit")
    if data["DR"]["val"].shape[0] != expected_control:
        raise AssertionError("Sequential control count does not match the data audit")
    if data["DR"]["damage"].shape[0] != expected_damage:
        raise AssertionError("Sequential damage count does not match the data audit")
    return data
...
def plot_all_orders_overlay(
    ...
    for i, result in enumerate(all_results.values()):
        order_label = "→".join(state_labels[key] for key in result["sequence"])
