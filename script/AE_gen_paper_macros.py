...
def gen_table_sequential_detection(language: str = "en") -> None:
    """Sequential adaptation: max-channel AUC grouped by final domain,
    with direct single-domain TL baselines for comparison."""
    ...
    FINAL_LABEL = {
        "en": {"DR": "Structural Repair", "SO": "Sensor Relocation", "SD": "Sensor Drift"},
        "cn": {"DR": "结构修复", "SO": "传感器迁移", "SD": "传感器漂移"},
    }
    ...
    for fi, final_dom in enumerate(FINAL_ORDER):
        ...
        for j, (key, val) in enumerate(entries):
            ordering_str = key  # already has Unicode arrows
            aucs = []
            for ed in EVAL_DOMAINS:
    ...
    if language == "en":
        header = r"Final domain & Ordering & Source & DR & SO & SD \\"
    else:
        header = r"最终域 & 排列顺序 & Source & DR & SO & SD \\"
...
def gen_table_training_comparison(language: str = "en") -> None:
    ...
    MODEL_COLS = [
        ("pretrain", {"en": "Pre-trained source", "cn": "预训练源域模型"}),
