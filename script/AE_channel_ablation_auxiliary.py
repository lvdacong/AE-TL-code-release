...
def plot_channel_trajectories_grid(
    ...
    # Reserve enough headroom for the in-panel difficulty label even when the
    # post-shift peak occurs at the most severe setting.
    global_vmax = max(vmx_global * 1.15, 1e-8)
    ...
    # A single shared label avoids three repeated long labels colliding after
    # the six-panel figure is reduced to manuscript column width.
    fig.supxlabel(r"$N_{\mathrm{tgt}}$", fontsize=fs_label)
