...
_XLABEL_TO_DIFF_LABEL = {
    ...
    "Drift Ratio": "$r$",
    "Temperature-deviation amplitude (°C)": r"$\Theta$ ($^\circ$C)",
}
...
def plot_difficulty_lines_2d(
    ...
    # Use the same canvas height and one-row legend in all three scenarios.
    is_temperature_span = xlabel == "Temperature-deviation amplitude (°C)"
    ...
    for j, diff in enumerate(difficulty_values):
        ...
        legend_label = (
            rf"$\Theta={diff}\,^{{\circ}}\mathrm{{C}}$"
            if is_temperature_span
            else f"{diff_label} = {diff}"
        )
        ax.plot(
            data_sizes, y_vals,
