...
- `script/review_checks/`: review-stage verification scripts for the non-neural baselines and noise-sensitivity CSV outputs used in the synthesis tables and figures.
...
The scripts keep the same relative layout as the working repository. When the requested finite-element data, preprocessed arrays, checkpoints, and review CSV outputs are placed under the expected folders, the code can regenerate the reported study outputs from the configured workflow.
